...
每篇文章的顶部都必须包含一段 YAML 格式的元数据，用于定义标题、日期和封面图等信息。
...
1. **本地预览**：运行 `npm run dev`，在浏览器中查看你的新文章。
   - 中文文章：`http://localhost:4321/blog/your-post-slug`
   - 英文文章：`http://localhost:4321/en/blog/your-post-slug`
2. **构建发布**：当你准备好发布时，运行 `npm run build` 生成静态文件。
